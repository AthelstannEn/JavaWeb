...
public class MadeCountry_Controller {
	...
	@PostMapping
	public String save(@ModelAttribute("madeCountry")@Valid Colors colors,BindingResult br, Model model, SessionStatus status, @PageableDefault Pageable pageable, @ModelAttribute("filter") TypeFilter filter){
		if(br.hasErrors()) return show(model, pageable, filter);
		goodService.save(colors);
		status.setComplete();
		return "redirect:/admin/goodCountries"+getParams(pageable, filter);
