...
public class GoodModelSpecifications implements Specification<Model> {
	...
	private void findByPrice(Root<Model> root, CriteriaQuery<?> query,
			CriteriaBuilder cb) {
	...
	private void findByModelName(Root<Model> root,
	...
	private void findByGoodType(Root<Model> root, CriteriaQuery<?> query) {
		if (!filter.getGoodTypesId().isEmpty()) {
			predicates.add(root.get("goodType").in(filter.getGoodTypesId()));
		}
	}

	private void findByGoodCountry(Root<Model> root,
			CriteriaQuery<?> query) {
